...
class RopaFormController extends Controller
{
    ...
    public function index(): View
    {
        ...
        $forms = $isAdmin
            ? RopaForm::with(['college', 'submissions'])->latest('updated_at')->get()
            : RopaForm::with(['college', 'submissions'])->where('user_id', $user->id)->latest('updated_at')->get();
    ...
    public function update(Request $request): RedirectResponse|JsonResponse
    {
        ...
        if ($step === 1) {
            ...
            if (! $basicInfoLocked) {
                ...
                $validationRules['business_function'] = 'required|string|max:255';
                $validationRules['main_process_name'] = 'required|string|max:255';
                $validationRules['has_sub_processes'] = 'nullable|boolean';
            }

            $validationRules['sub_process_name'] = 'nullable|string|max:255|required_if:has_sub_processes,1';

            $validated = $request->validate($validationRules);

            if (! $basicInfoLocked) {
                // Save basic info on the parent form (only on first sub-process)
                $parentForm->update([
    ...
    {
        $user = Auth::user();
        $isOwner = $parentForm->user_id === $user->id;
        $isAdmin = $user->role === 'admin';
...
/**
    ...
    public function viewSubmission(RopaSubmission $submission): View
    {
        ...
        // Allow access if: user owns the form OR user is admin
        $isOwner = $parentForm && $parentForm->user_id === $user->id;
        $isAdmin = $user->role === 'admin';

        if (! $parentForm || (! $isOwner && ! $isAdmin)) {
